...
export async function listMeetingsForUser(req) {
  let query = supabase
    .from("meetings")
    .select(
...
export async function listMeetingsByEmployeeId(employeeId) {
  const { data, error } = await supabase
    .from("meetings")
    .select(
      "id, project_name, client_name, project_type, upwork_account, duration_minutes, budget_discussed, deadline, created_at",
    )
    .eq("employee_id", employeeId)
